...
const formatPost = (post) => {
    const media = post.mediaUrl
        ...
        : null;
    const likesArray = (post.likes || []).map((l) => l.userId);
    const repostsArray = (post.reposts || []).map((r) => r.userId);
...
const createPost = async (userId, description, file) => {
    if ((!description || description.trim() === "") && !file) {
        throw new AppError(status.BAD_REQUEST, "Please add a description or media to your post");
    }
...
const getAllPosts = async (filters, pagination) => {
...
const getUserPosts = async (targetUserId) => {
...
const addComment = async (userId, postId, payload) => {
    ...
    if (post.userId !== userId) {
        const actor = await prisma.user.findUnique({ where: { id: userId } });
        if (actor) {
            const truncated = text.trim().substring(0, 50) + (text.trim().length > 50 ? "..." : "");
...
const editComment = async (userId, postId, commentId, text) => {
...
const deleteComment = async (userId, userRole, postId, commentId) => {
    const comment = await prisma.comment.findUnique({
        where: { id: commentId },
    });
    if (!comment || comment.postId !== postId) {
...
const toggleInterested = async (userId, postId) => {
...
const toggleNotInterested = async (userId, postId) => {
...
const updatePost = async (userId, userRole, postId, description) => {
    const post = await prisma.post.findUnique({
        where: { id: postId, isActive: true },
    });
    if (!post) {
...
const deletePost = async (userId, userRole, postId) => {
    const post = await prisma.post.findUnique({
        where: { id: postId, isActive: true },
    });
    if (!post) {
