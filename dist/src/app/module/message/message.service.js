...
let emitMessageCallback = null;
export const setMessageSocketEmitter = (callback) => {
...
const sendMessage = async (senderId, receiverId, payload) => {
    const { message = "", messageType = "text", mediaUrl = null, fileName = null, fileSize = null, tempId, } = payload;
    ...
    // Format preview text
    const preview = messageType === "share"
        ? "📱 Shared a post"
        : message ||
            (messageType === "image"
                ? "📷 Photo"
...
const getConversations = async (userId) => {
    ...
    const conversations = await Promise.all(friendships.map(async (f) => {
